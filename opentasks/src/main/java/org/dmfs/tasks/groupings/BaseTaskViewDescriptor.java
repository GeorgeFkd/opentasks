...
public abstract class BaseTaskViewDescriptor implements ViewDescriptor
{
    ...
    protected void setPrio(SharedPreferences prefs, View view, Cursor cursor)
    {
                ...
                prefs.getBoolean(prioLabel.getContext().getString(R.string.opentasks_pref_appearance_list_show_priority), true))
        {
            if (priority < 5)
            {
                prioLabel.setBackgroundColor(new AttributeColor(prioLabel.getContext(), org.dmfs.tasks.theme.R.attr.colorHighPriority).argb());
            }
            if (priority == 5)
            {
                prioLabel.setBackgroundColor(new AttributeColor(prioLabel.getContext(), org.dmfs.tasks.theme.R.attr.colorMediumPriority).argb());
            }
            if (priority > 5)
            {
                prioLabel.setBackgroundColor(new AttributeColor(prioLabel.getContext(), org.dmfs.tasks.theme.R.attr.colorLowPriority).argb());
            }
    ...
    protected void setColorBar(View view, Cursor cursor)
    {
        ...
        if (cardView != null)
        {
            ...
            cardView.setCardElevation(view.getResources().getDimensionPixelSize(
                    isClosed ?
                            org.dmfs.tasks.theme.R.dimen.opentasks_tasklist_card_elevation_closed :
                            org.dmfs.tasks.theme.R.dimen.opentasks_tasklist_card_elevation));
